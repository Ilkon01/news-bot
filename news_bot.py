...
def deduplicate(items, threshold, recent):
    ...
    winners = []
    for g in groups:
        g[0]["sources_count"] = len(g)
        g[0]["also_in"] = [x["source"] for x in g[1:]][:4]
        winners.append(g[0])
...
def build_prompt(item, config):
    ...
    rules = config.get("editorial_rules", [])
...
ТЕМЫ ЧИТАТЕЛЯ (3 = присылать обязательно, 2 = присылать если новость
...
2. topic — ровно одно название темы из списка выше.
...
4. importance — ровно одно слово: СРОЧНО или ОБЫЧНО.
   ...
   Это событие, о котором человек захочет узнать немедленно, не дожидаясь
   вечера, и которое меняет положение дел:
   ...
   ВО ВСЕХ ОСТАЛЬНЫХ СЛУЧАЯХ — ОБЫЧНО. Очередной эпизод продолжающегося
   конфликта, заявление политика, аналитика, отчёт, назначение, законопроект,
   переговоры без результата, данные статистики — это ОБЫЧНО, даже если
   тема имеет приоритет 3.
5. title_ru — заголовок на русском, до 10 слов, без точки в конце.
   ВАЖНО: это не перевод, а самостоятельный заголовок. Пиши так, как
   написал бы редактор русскоязычного издания: живым, естественным
   языком, который читается с первого раза.
   - Не переводи английские обороты буквально. «Officials said» — это
     не «официальные лица сказали», а просто «власти сообщили».
   ...
   - Используй глаголы вместо отглагольных существительных. Плохо:
     «Проведение переговоров о прекращении поставок». Хорошо:
     «Стороны обсудили прекращение поставок».
   - Имена и названия пиши в принятой русской форме: Netanyahu —
     Нетаньяху, Erdogan — Эрдоган, Reuters — Reuters (латиницей).
   - Не оставляй в заголовке английские слова, кроме названий компаний
     и изданий.
   - Не начинай с предлога или вводного оборота, начинай с сути.
...
lead и detail пиши тем же живым русским языком: короткие фразы,
глаголы вместо канцелярита, никакой кальки с английского.
Не переводи дословно и не копируй фразы из оригинала. Без вводных вроде «В статье говорится» и без названия
...
def format_message(item, data, emoji_map, show_hashtags=True, lead_max=140):
    ...
    if n > 1:
        also = ", ".join(item.get("also_in", []))
        line = f"✅ Подтверждено: {n} источника"
        if also:
